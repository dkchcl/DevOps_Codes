...
module "vm" {
  depends_on          = [module.rg, module.nic1, module.kv, module.kvs]
  ...
  offer               = var.offer
  sku                 = var.sku
  os_disk_name        = var.f_os_disk_name
  caching             = var.caching
  create_option       = var.create_option
  managed_disk_type   = var.managed_disk_type
  computer_name       = var.f_computer_name
...
module "vm1" {
  depends_on          = [module.rg, module.nic2, module.kv, module.kvs]
  source              = "../Modules/Virtual_machine"
  resource_group_name = var.resource_group_name
  location            = var.location
  nic_name            = var.b_nic_name
  ...
  offer               = var.offer
  sku                 = var.sku
  os_disk_name        = var.b_os_disk_name
  caching             = var.caching
  create_option       = var.create_option
  managed_disk_type   = var.managed_disk_type
  computer_name       = var.b_computer_name
  key_vault           = var.kv_name
  username            = var.username
  password            = var.password
}
